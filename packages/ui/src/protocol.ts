...
export function decodeBinaryDiff(buffer: Uint8Array | ArrayBuffer): TerminalDiff {
  ...
  if (cursorShapeRaw === 1) cursorShape = 'underline';
  else if (cursorShapeRaw === 2) cursorShape = 'bar';
  ...
  for (let r = 0; r < numDirtyRows; r++) {
    ...
    for (let c = 0; c < cols; c++) {
      if (offset + CELL_SIZE > bytes.byteLength) break;
...
function decodeColor(raw: number): TerminalColor {
