...
export class VtEngine {
  public cols: number;
  public rows: number;
  public maxScrollback: number = 10000;

  public lines: TerminalCell[][] = [];
  public scrollback: TerminalCell[][] = [];
  public viewportY: number = 0; // 0 to scrollback.length
  ...
  private savedCursor = { col: 0, row: 0 };
  private scrollTop: number = 0;
  private scrollBottom: number = 23;

  private currentFg: TerminalColor = { type: 'default' };
  private currentBg: TerminalColor = { type: 'default' };
  private currentFlags: number = CellFlags.EMPTY;

  private dirtyRowSet = new Set<number>();
  private events: VtEvents;

  // Parser state
  private state: 'GROUND' | 'ESC' | 'CSI' | 'OSC' | 'OSC_ESC' = 'GROUND';
  private csiParams: string = '';
  private oscBuffer: string = '';

  constructor(cols: number = 80, rows: number = 24, events: VtEvents = {}) {
  ...
  public scrollLines(delta: number) {
    ...
    if (prev !== this.viewportY) {
      for (let r = 0; r < this.rows; r++) this.dirtyRowSet.add(r);
    }
  }
  ...
  public getVisibleLines(): TerminalCell[][] {
    const visible: TerminalCell[][] = [];
  ...
  private createEmptyLine(): TerminalCell[] {
    const line: TerminalCell[] = new Array(this.cols);
  ...
  private handleCsi(finalChar: string) {
    ...
    switch (finalChar) {
      ...
      case 'h': // Set Mode
        if (isPrivate) {
          if (args.includes(25)) this.cursor.visible = true;
        }
        break;

      case 'l': // Reset Mode
        if (isPrivate) {
          if (args.includes(25)) this.cursor.visible = false;
        }
        break;
    }
  }
  ...
  private handleSgr(args: number[]) {
    ...
    while (i < args.length) {
      ...
      } else if (code === 1) this.currentFlags |= CellFlags.BOLD;
      else if (code === 2) this.currentFlags |= CellFlags.DIM;
      else if (code === 3) this.currentFlags |= CellFlags.ITALIC;
      else if (code === 4) this.currentFlags |= CellFlags.UNDERLINE;
      else if (code === 5) this.currentFlags |= CellFlags.BLINK;
      else if (code === 7) this.currentFlags |= CellFlags.INVERSE;
      else if (code === 8) this.currentFlags |= CellFlags.HIDDEN;
      else if (code === 9) this.currentFlags |= CellFlags.STRIKETHROUGH;
      else if (code === 22) this.currentFlags &= ~(CellFlags.BOLD | CellFlags.DIM);
      else if (code === 23) this.currentFlags &= ~CellFlags.ITALIC;
      else if (code === 24) this.currentFlags &= ~CellFlags.UNDERLINE;
      else if (code === 25) this.currentFlags &= ~CellFlags.BLINK;
      else if (code === 27) this.currentFlags &= ~CellFlags.INVERSE;
      else if (code === 28) this.currentFlags &= ~CellFlags.HIDDEN;
      else if (code === 29) this.currentFlags &= ~CellFlags.STRIKETHROUGH;
  ...
  private handleOsc(buffer: string) {
  ...
  private clearLineRange(row: number, startCol: number, endCol: number) {
    if (row < this.lines.length) {
