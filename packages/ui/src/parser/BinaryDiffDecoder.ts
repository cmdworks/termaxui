...
export class BinaryDiffDecoder {
  public static decode(buffer: ArrayBuffer | Uint8Array): TerminalDiff | null {
    ...
    if (shapeVal === 1) cursorShape = 'underline';
    else if (shapeVal === 2) cursorShape = 'bar';
    ...
    for (let i = 0; i < numDirtyRows; i++) {
      ...
      for (let c = 0; c < cols; c++) {
        ...
        let fg: TerminalColor = { type: 'default' };
        if (fgType === 1) {
        ...
        let bg: TerminalColor = { type: 'default' };
        if (bgType === 1) {
